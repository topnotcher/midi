...
static inline void midi_parse_hdr(FILE *, midi_hdr_t *);
static inline void midi_parse_track(FILE *, midi_track_t * );
static void midi_parse_track_hdr(FILE *, midi_track_hdr_t *);
...
static inline void midi_parse_hdr(FILE *file, midi_hdr_t *hdr) {
	fread(hdr, MIDI_HEADER_SIZE, 1, file);
...
static void midi_parse_track_hdr(FILE *file, midi_track_hdr_t *hdr) {
	fread(hdr, MIDI_TRACK_HEADER_SIZE, 1, file);
...
static inline midi_event_node_t *midi_parse_event(FILE *file, unsigned int *const bytes) {
	...
	*bytes += fread(&cmdchan, 1,1, file);
	...
	if ( cmdchan == 0xFF )  {
		...
		//skip the command.
		*bytes += fread(&cmd, 1,1, file);
		*bytes += fread(&size, 1,1, file);
